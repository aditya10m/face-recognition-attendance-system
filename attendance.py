...
class Attendance:
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update std_attendance set std_id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
        ...
        if self.var_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(username='root', password='Aditya@123',host='localhost',database='face_recognizer1',port=3306)
                    mycursor = conn.cursor() 
                    sql="delete from std_ID where std_id=%s"
                    val=(self.var_id.get(),)
    ...
    def fetch_data(self):
        ...
        mycursor.execute("select * from std_ID")
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = mysql.connector.connect(username='root', password='Aditya@123',host='localhost',database='face_recognizer1',port=3306)
                mycursor = conn.cursor()
                mycursor.execute("insert into std_ID values(%s,%s,%s,%s,%s,%s)",(
                self.var_id.get(),
                self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
        ...
        if messagebox.askyesno("Confirmation","Are you sure you want to save attendance on database?"):
             for i in mydata:
                 ...
                 udate = i[4]
                 uattend = i[5]
                 qury = "INSERT INTO std_id(std_id, std_roll_no, std_name, std_time, std_date, std_attendance) VALUES(%s,%s,%s,%s,%s,%s)"
